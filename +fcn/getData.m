function [subjInfo, ds, dp] = getData(stg, dsDesc, dpDesc, lblp, snlp, dobTable, ksubj, subjNmOrig)
    ...
    [subjNm, anStartDt, anEndDt, chName] = gd.dbfGetSubjInfo(stg, lblpn, subjNmOrig);
    ...
    for kn = 1 : numel(dsDesc.Name)
        ...
        ds.(nm) = dt.tblSetTimeZone(ds.(nm), stg.recTimeZoneStr);
    ...
    for kbinlen = 1 : numel(binlenUnDu)
        ...
        binDt = (anStartDt : binlenUnDu(kbinlen) : anEndDt)'; % Edges of bins in UTC datetime
        ...
        for kb = 1 : numbin % Loop over time blocks
            ...
            tol = seconds(0.00); % Tolerance in seconds
            ...
            for klf = 1 : numel(lblfSub) % k-th label file (out of those relevant for this block)
                if loadedLblpn ~= string(lblpn{lblfSub(klf)}) % Check if the required data file is already loaded. If not, load it.
                    ...
                    % Check if new file begins after the end of the previous file
                    if exist('prevSigInfoUTC', 'var')
                        dt.checkIfStartIsAfterPrevEnd(ll, prevSigInfoUTC, lblpn, lblfSub(klf), 60)
                    end
                    prevSigInfoUTC = ll.sigInfo;
                    % End of curation and checks
            ...
            for kn = 1 : numel(dpDesc.Name) % Over the names of the phenomena
                ...
                if dd(1).BinLenDu == binlenUnDu(kbinlen)
                    ...
                    if validS > seconds(dpDesc.(nm)(1).BinLenDu)*1.01 % We allow 1% inaccuracy
                        disp(['y = ', num2str(y)])
                        disp(['BinLenDu = ', num2str(seconds(dpDesc.(nm)(1).BinLenDu))])
                        disp(['validS = ', num2str(validS)])
